...
class ICICIParser(BaseBankParser):
    ...
    def extract_account_info(self, full_text: str) -> BankInfo:
        ...
        ifsc_match = re.search(r"IFSC[:\s]*(ICIC\d{7})", full_text, re.I)
        if ifsc_match:
    ...
    def _find_header(self, table: list[list[str | None]]) -> tuple[int | None, dict[str, int]]:
        kw = {
            ...
            "narration": ["particulars", "narration", "description", "details"],
            ...
            for col_name, keywords in kw.items():
                for ci, cell in enumerate(row):
                    if any(k in (cell or "").strip().lower() for k in keywords):
                        col_map[col_name] = ci
                        break
            if "date" in col_map and ("narration" in col_map or "withdrawal" in col_map):
                return idx, col_map
        return None, {}
